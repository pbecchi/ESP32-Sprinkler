...
static unsigned char htmlMobileHeader[] /*PROGMEM*/ =
    "<meta name=\"viewport\" content=\"width=device-width,initial-scale=1.0,minimum-scale=1.0,user-scalable=no\">\r\n"
    ;

static unsigned char htmlReturnHome[] /*PROGMEM*/ =
...
#ifdef OPENSPRINKLER_ARDUINO // n.b. h2int is also already defined in ethercard->webutil.cpp
/** Convert a single hex digit character to its integer value */
unsigned char h2int_arduino ( char c )
{
    if ( c >= '0' && c <='9' )
    {
...
void urlDecode ( char *urlbuf )
{
    ...
    while ( ( c = *urlbuf ) != 0 )
    {
        ...
        if ( c == '%' )
        {
            c = *++urlbuf;
#ifdef OPENSPRINKLER_ARDUINO
            c = ( h2int_arduino ( c ) << 4 ) | h2int_arduino ( *++urlbuf );
#else
 ...
 * name:  program name
*/
static char _str_program[] /*PROGMEM*/ = "Program ";
		    ...
		    if (oid==OPTION_MASTER_OFF_ADJ || oid==OPTION_MASTER_OFF_ADJ_2 ||
		      ...
		      v=water_time_encode_signed(v);
				...
				if (v >= 0 && v <= max_value) {
					...
					err = 1;
					DEBUG_PRINTLN(); DEBUG_PRINT(oid); DEBUG_PRINT(" "); DEBUG_PRINT(max_value); DEBUG_PRINT(">");DEBUG_PRINTLN(v);
				}
			}
		}
...
///t    write_to_file(ifkey_filename, tmp_buffer, strlen(tmp_buffer));
  } else if (keyfound) {
    tmp_buffer[0]=0;
///t    write_to_file(ifkey_filename, tmp_buffer, strlen(tmp_buffer));
  }  
...
void handle_web_request(char *p) {
	...
	if (com[0] == ' ' || com[0] == 0) {
		...
		if (i == sizeof(urls) / sizeof(URLHandler)) {
			DEBUG_PRINTLN("no server function");
...
//const int NTP_PACKET_SIZE = 48; // NTP time is in the first 48 bytes of message
	...
	time_t getNtpTime() {
		Udp.begin(8888); DEBUG_PRINTLN(WiFi.dnsIP());
		byte itry = 0;
		while (itry++ < 20) {
			...
			//		if (!ether.HOSTname(timeServer, "time.nist.gov")) {
			//			timeServer = IPAddress(os.options[OPTION_NTP_IP1],
...
time_t getNtpTime()
{
	...
	IPAddress serverIp(ntpip[0], ntpip[1], ntpip[2], ntpip[3]);
	...
	do
	{
		...
		expire = millis() + 4000; // wait for at most 4 second  
		do
		{
		...
		tick++;
		WiFi.hostByName("time.nist.gov", serverIp);
	} while (tick < 20);
	return 0;
}
#endif
