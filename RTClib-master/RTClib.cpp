...
boolean DateTime::summertime_EU(const uint8_t tzHours) {
...
//
static uint8_t bcd2bin(uint8_t val) { return val - 6 * (val >> 4); }//  return ((num/10 * 16) + (num % 10));
static uint8_t bin2bcd(uint8_t val) { return val + 6 * (val / 10); }// return ((num / 16 * 10) + (num % 16));
static uint8_t dec2bcd(uint8_t num) { return ((num / 10 * 16) + (num % 10)); }
static uint8_t bcd2dec(uint8_t num) { return ((num / 16 * 10) + (num % 16)); }
boolean RTC_MCP7940::use_summertime_EU = false;
uint8_t RTC_MCP7940::begin(void) {
	WIRE.beginTransmission(RTC_ADDRESS);
	if (WIRE.endTransmission()==0)return 1;
	return 0;
}
#define D_P(x) Serial.println(x)
...
void RTC_MCP7940::adjust(const DateTime& dt) {
	...
	WIRE.beginTransmission(RTC_ADDRESS);
	WIRE._I2C_WRITE(0);
	WIRE._I2C_WRITE(bin2bcd(d.second()) | 0x80);
...
bool RTC_MCP7940::write(tmElements_t &d)
{
	//if (use_summertime_EU && d.summertime_EU(0)) d = d - TimeSpan(3600);
	WIRE.beginTransmission(RTC_ADDRESS);
	WIRE._I2C_WRITE(0);
	WIRE._I2C_WRITE(dec2bcd(d.Second) | 0x80);
	WIRE._I2C_WRITE(dec2bcd(d.Minute));
	WIRE._I2C_WRITE(dec2bcd(d.Hour));

	(RTC_BATT_HIGH) ? WIRE._I2C_WRITE(bin2bcd(0) | 0x08)
		: WIRE._I2C_WRITE(bin2bcd(0));

	WIRE._I2C_WRITE(dec2bcd(d.Day));
	WIRE._I2C_WRITE(dec2bcd(d.Month));
	WIRE._I2C_WRITE(dec2bcd(d.Year+1970 - 2000));
	WIRE._I2C_WRITE(0);
	WIRE.endTransmission();
	return true;
}
bool RTC_MCP7940::read(tmElements_t &dm) {
	{
		WIRE.beginTransmission(RTC_ADDRESS);
		WIRE._I2C_WRITE(0);
		WIRE.endTransmission();
		WIRE.requestFrom(RTC_ADDRESS, 7);
		dm.Second = bcd2dec(WIRE._I2C_READ() &0x7F);
		dm.Minute = bcd2dec(WIRE._I2C_READ());
		dm.Hour = bcd2dec(WIRE._I2C_READ());  // mask assumes 24hr clock
		dm.Wday = (bcd2dec(WIRE._I2C_READ()&0x07)+1)%7;// CORRECT TO WEEKDAY DEFINITION
		dm.Day = bcd2dec(WIRE._I2C_READ());
		dm.Month = bcd2dec(WIRE._I2C_READ() &0x1F); //exclude Leapyear on bit 5 onLY ON MCP7940 !!!
		dm.Year = bcd2dec(WIRE._I2C_READ())+30;    //CORRECT FOR YEARS FROM 1970
		//if (use_summertime_EU && dt.summertime_EU(0)) return  dt + TimeSpan(3600);
		return true;
	}
}

Ds1307SqwPinMode RTC_MCP7940::readSqwPinMode() {
	int mode;
...
void RTC_MCP7940::writeSqwPinMode(Ds1307SqwPinMode mode) {
	WIRE.beginTransmission(RTC_ADDRESS);
	WIRE._I2C_WRITE(DS1307_CONTROL);
...
////////_PCFRTC_DS1307 implementation

//static uint8_t bcd2bin (uint8_t val) { return val - 6 * (val >> 4); }
//static uint8_t bin2bcd (uint8_t val) { return val + 6 * (val / 10); }

// Aquire data from the RTC chip in BCD format
bool RTC_DS1307::read(tmElements_t &tm)
{
	uint8_t sec;
	...
	Wire.requestFrom(DS1307_ADDRESS, tmNbrFields);
...
bool RTC_DS1307::write(tmElements_t &tm)
{
	Wire.beginTransmission(DS1307_ADDRESS);
#if ARDUINO >= 100  
...
bool RTC_PCF8523::read(tmElements_t &tm)
{
	uint8_t sec;
	Wire.beginTransmission(PCF8523_ADDRESS);
#if ARDUINO >= 100  
...
bool RTC_PCF8523::write(tmElements_t &tm)
{
	Wire.beginTransmission(PCF8523_ADDRESS);
#if ARDUINO >= 100  
	Wire.write((uint8_t)0x00); // reset register pointer  
